...
function findImageUrl(channel: Element): string | null {
...
function parseEpisode(item: Element): Episode | null {
...
async function fetchFeed(feedUrl: string): Promise<string> {
  try {
    ...
    throw new FeedError(`Échec de la récupération du flux: ${(error as Error).message}`);
  }
}

function parseFeed(xmlText: string): Omit<FeedData, "feed_url" | "updated_at"> {
...
export async function subscribeToFeed(feedUrl: string): Promise<FeedData> {
